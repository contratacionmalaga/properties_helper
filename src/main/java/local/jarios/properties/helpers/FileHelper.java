...
public final class FileHelper {
    ...
    public static boolean isInvalidDirectory(File directory) {
        ...
        if (!directory.isDirectory()) {
            LOGGER.debug("[isInvalidFile] - El directorio no es un Directorio: {}", directory.getAbsolutePath());
            return true;
        }

        if (!directory.canRead()) {
            LOGGER.debug("[isInvalidFile] - El directorio no se puede leer: {}", directory.getAbsolutePath());
            return true;
        }
